...
const logger = createLogger('openfoodfacts')
...
export async function importOpenFoodFacts(options: ImportOptions): Promise<void> {
  logger.info('Starting Open Food Facts CSV import')

  const progress = new ProgressTracker('openfoodfacts')
  const importLog = new ImportLogger('openfoodfacts')
